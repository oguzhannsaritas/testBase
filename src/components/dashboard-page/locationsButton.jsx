...
const locations = [
    {
        ...
        subLocations: ["USA", "Canada", "Mexico", "USA", "Canada", "Mexico", "USA", "Canada", "Mexico"]
    },
    {
        name: "South America - West Coast",
        subLocations: ["Brazil", "Argentina", "Chile"]
    },
    {
        name: "Europe - West Coast",
        subLocations: ["Portugal", "Spain", "France", "Ireland", "United Kingdom"]
    },
    // ...
];

export function LocationsButton({ onAddStep }) {
    ...
    // Sublocation'a tıklayınca TestPanel'e ekle
    const handleSubLocationClick = (sublocation) => {
        onAddStep("Locations", sublocation); // Sadece ülke adını ekliyoruz
    };


    return (
        <div
            className="relative w-32 min-w-32 top-[112px] left-[61%] translate-x-[400%]"
            ref={menuRef}
        >
            <button
            ...
            <AnimatePresence>
                ...
                {selectedLocation || "Location"}
                        ...
                        <div
                            className="-top-1 mb-[6px] flex relative
                w-[200px] h-[2px] flex-shrink-0 mx-auto
                rounded-[10px]
                        ...
                        <div className="flex flex-col space-y-2 text-sm pr-[15px] pl-2 max-h-60 overflow-y-auto">
                            {activeMenu === "main"
                                ? locations.map((location, index) => (
                                    ...
                                    >
                                        {location.name} <ChevronRight size={16} />
                                    </button>
                                ))
                                : locations
                                    .find((loc) => loc.name === activeMenu)
                                    ?.subLocations.map((sublocation, subIndex) => (
                                        <button
                                            key={subIndex}
